...
@foreach($gedung as $g)                                               
       ...
       @if(count($slot) > 0)
       <a href="/opslotoperator/{{$l->id}}/tambah10" class="btn btn-primary">Add</a>
</form>
<div class="d1">
        <a href="#deleteop10" data-toggle="modal" class="btn btn-danger">Delete Slot</a>
        @include('inc.modal1')
        
</div>
    <div class="d2">
     
    <a href="#deleteopfull" data-toggle="modal" class="btn btn-danger">Delete Layout</a>
     @include('inc.modal2')
</div>  
       @else
...
<!--Cetak List Lantai -->
    @foreach($lantai1 as $l1)
        @if($l1->id == $l->id)          
            <a href="/lantaioperator/{{$l1->id}}"><div class="btn btn-danger">{{$l1->Nama_Lantai}}</div></a>
        @else
            <a href="/lantaioperator/{{$l1->id}}"><div class="btn btn-primary">{{$l1->Nama_Lantai}}</div></a>
        @endif
    @endforeach
...
<div class="isi">       
    <!--Cetak Slot Lantai -->
        @foreach($slot as $sub)
                    ...
                    <a href="/opslotoperator/layout/{{$sub->id}}"><div class="kotak"></div></a>
